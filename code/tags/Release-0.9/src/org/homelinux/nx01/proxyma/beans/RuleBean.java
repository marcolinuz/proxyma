 ...
 * Time: 21.33.15
 * </p><p>
 * This class is the configuration bean for an instance of a reverse proxy destination.
 * </p><p>
...
public class RuleBean implements Serializable {
    ...
    public void setProxyPassHost(String proxyPassHost) throws NullPointerException, IllegalArgumentException {
        ...
        try {
            ...
            //Set the domain of the resource (for cookie rewriting)
            String host = parseUrl.getHost();
            if (host.indexOf(ProxymaConstants.DOT) >= 0)
                this.ruleDomain = host.substring(host.indexOf(ProxymaConstants.DOT));
            else
                this.ruleDomain = host;
    ...
    public void setLoggingPolicy(int loggingPolicy) {
        ...
        switch (this.loggingPolicy) {
            case ProxymaConstants.LOGGING_POLICY_NONE:
                this.debugLoggingEnabled = false;
                this.productionLoggingEnabled = false;
                this.performanceTestEnabled = false;
                this.onlineDebugDisabled = true;
                break;

            case ProxymaConstants.LOGGING_POLICY_PRODUCTION:
                this.debugLoggingEnabled = false;
            ...
            case ProxymaConstants.LOGGING_POLICY_PERFORMANCE_TEST:
                this.debugLoggingEnabled = false;
                this.productionLoggingEnabled = true;
                this.performanceTestEnabled = true;
                this.onlineDebugDisabled = true;
                break;
    ...
    /**
     * Returns true if the logging policy needs to generate a performance test.
     *
     * @return true if the logging policy needs to generate a performance test.
     */
    public boolean isPerformanceTestEnabled() {
        return performanceTestEnabled;
    }
    ...
    public void setJavascriptHandlingMode(int javascriptHandlingMode) {
        ...
        switch (javascriptHandlingMode) {
            case RewriterConstants.PROCESS:
    ...
    /**
     * Obtain the status of the cache for the rule. If this is set to FALSE the rule will not use
     * the cache subsystem and ALL the requests will follow the same path:
     * Client -> proxyma -> Server -> proxyma -> client.
     * If this flag is set to TRUE, proxyma will search into its cache subsystem a resource that matches the wanted url.
     * If the resource is found the flow of the request will be faster:
     * Client -> Proxyma (chache) -> Client.
     *
     * @return the status of the flag for this rule
     */
    ...
    /**
     * Set a new value for the flag. If this is set to FALSE the rule will not use
     * the cache subsystem and ALL the requests will follow the same path:
     * Client -> proxyma -> Server -> proxyma -> client.
     * If this flag is set to TRUE, proxyma will search into its cache subsystem a resource that matches the wanted url.
     * If the resource is found the flow of the request will be faster:
     * Client -> Proxyma (chache) -> Client.
     *
     * @param chacheEnabled a boolean value.
     */
    public void setCacheEnabled(boolean chacheEnabled) {
        this.cacheEnabled = chacheEnabled;
    }

    /**
     * Obtains the maximum size of a cached resource for this rule.
     *
     * @return the number of currently allowed bytes.
     */
    public int getMaxCachedResourceSize() {
        return maxCachedResourceSize;
    }

    /** Sets the maximum size of a cached resource for this rule
     *
     * @param maxCachedResourceSize the number of allowed bytes.
     */
    public void setMaxCachedResourceSize(int maxCachedResourceSize) {
        if (maxCachedResourceSize >= 0)
            this.maxCachedResourceSize = maxCachedResourceSize;
    }

    /**
     * Obiain the domain of the resource
     * @return the domain of the proxed resource
     */
    ...
    public String toString() {
        ...
        retValue.append("\n\tMasqueraded Domain = " + getRuleDomain());
        ...
        retValue.append("\n\tCache Status = " + (isRewriteEngineEnabled() ? "Enabled" : "Disabled"));
        retValue.append("\n\tMax size of a cached resource = " + getMaxCachedResourceSize());
        retValue.append("\n\tAudit Mode = " + ProxymaConstants.LOGGING_POLICY_STRING[getLoggingPolicy()]);
        retValue.append("\n\tJavascript Handling Mode = " + ((getJavascriptHandlingMode() == RewriterConstants.PROCESS) ? "Rewrite Javascript Urls" : "Remove Javascript Code"));
    ...
    private boolean performanceTestEnabled = false;
    ...
    //Set if the rule is authorized to use the Proxyma ResourceCache Subsystem
    private boolean cacheEnabled = ProxymaConstants.DEFAULT_CACHE_STATUS;

    //Max size (in bytes) of a Cached Resource
    private int maxCachedResourceSize = ProxymaConstants.DEFAULT_MAX_CACHED_RESOURCE_SIZE;

    //the domain of the rule for cookie rewriting
