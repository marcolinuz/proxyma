...
public class RewriterEngine {
    ...
    public void rewriteLinks(RuleBean rule, RuleSet ruleSet, ResponseResourceBean responseResource) throws ParserException, IOException {
            ...
            public void visitTag(Tag tag) {
                ...
                if (RewriterConstants.A.equalsIgnoreCase(name)) {
                    ...
                    if (tagValue != null)
                        ...
                        if ((Language != null) && (Language.toLowerCase().indexOf(RewriterConstants.JAVASCRIPT_LIBRARY) >= 0))
        ...
        String encoding = responseResource.getEncodingCharset();
        if (ProxymaConstants.UNKNOWN_CHARSET.equals(encoding))
        	encoding = guessEncodingFromMetaTags(responseResource);

        SmartBufferReader reader = new SmartBufferReader(responseResource.getData());
        String content = new String(reader.getBytes(), encoding);
    ...
    /**
     * Find Http-Equiv META tags to guess the charset encoding by visit the html data.
     * This method is caled only if the server doesn“t provieds the charset encoding
     * into the response data.
     *
     * @param responseResource is the wrapped original response of the remote server.
     */
    private String guessEncodingFromMetaTags(ResponseResourceBean responseResource) {
        String result = ProxymaConstants.UNKNOWN_CHARSET;
        final Pattern charsetReplaceEncodingPattern = Pattern.compile("^.*=");

    	try {
	         SmartBufferReader reader = new SmartBufferReader(responseResource.getData());
	         String content = new String(reader.getBytes(), ProxymaConstants.DEFAULT_CHARSET);
	         Parser parser = new Parser(new Lexer(new Page(content, ProxymaConstants.DEFAULT_CHARSET)));
	         ...
	         NodeFilter filter = new AndFilter (
	                 ...
	                 new NodeFilter () {
	                     ...
	                     {
	                         return (RewriterConstants.CONTENT_TYPE.equals(((MetaTag)node).getHttpEquiv()));
	                     }
	                 }
	             );
	         NodeList list = parser.extractAllNodesThatMatch(filter);
	         for (int i = 0; i < list.size (); i++) {
	                String metaContent = ((MetaTag)(list.elementAt(i))).getMetaContent();
	                result = charsetReplaceEncodingPattern.matcher(metaContent).replaceFirst(ProxymaConstants.EMPTY_STRING);
	                if ((result.length() == metaContent.length()) && (result.indexOf("/") >= 0)) {
	                    result = ProxymaConstants.DEFAULT_CHARSET;
	                    i = list.size();
	                }
	         }
    	} catch (Exception e) {
