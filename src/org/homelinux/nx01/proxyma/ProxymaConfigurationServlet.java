 ...
 *
 *
 * todo: split proxyma into proxy and console applications
 *
...
public class ProxymaConfigurationServlet extends HttpServlet {
    ...
    public void init() throws ServletException {
        ...
        //Load the configuration parameter for the configuration servlet.
        //Any error will be noticed into the server log. (catalina.out or so on..)
        ...
        if ((httpReverseProxyServletSubPath == null) || (ProxymaConstants.EMPTY_STRING.equals(httpReverseProxyServletSubPath)))
            System.out.println("[ProxymaConfigurationServlet.init()] WARNING! Configuration parameter \"" + ConfigurationConstants.httpReverseProxyServletSubPath + "\" was not found, So the Configuration pages Will not work properly!!");
    }
    ...
    public void doGet(HttpServletRequest httpservletrequest, HttpServletResponse httpservletresponse)
        ...
        RuleSet ruleSet = ProxymaFacade.getRuleSet(proxymaInstanceContext);
        ...
        if (ConfigurationConstants.updateRule.equals(method)) {
            ...
            if (ConfigurationConstants.enable.equals(action)) {
                ...
                RuleBean rule = ruleSet.getRule(target);
                if (rule != null) {
            ...
            if (ConfigurationConstants.addNewRule.equals(action)) {
                ...
                if (newRule != null) {
                    try {
                        RuleBean oldRule = ruleSet.getRule(target);
     ...
     */
    private RuleBean buildRuleFromRequest(HttpServletRequest request)
        ...
        String maxCachedResourceSize = request.getParameter(ConfigurationConstants.maxCachedResourceSize);
        String isCacheEnabled = request.getParameter(ConfigurationConstants.isCacheEnabled);
        ...
        String loggingPolicy = request.getParameter(ConfigurationConstants.loggingPolicy);

        return ProxymaFacade.getNewCustomRule(proxyFolder,
                proxyPassHost,
                proxyUser,
                proxyPassword,
                loggingPolicy,
                ...
                newContext,
                maxPostSize,
                maxCachedResourceSize,
                (isCacheEnabled != null) ? ProxymaConstants.TRUE : ProxymaConstants.FALSE,
     ...
     * @param ruleSet
     */
    private void enableRule(String target, RuleSet ruleSet) {
        RuleBean rule = ruleSet.getRule(target);
        if (rule != null)
     ...
     * @param ruleSet
     */
    private void disableRule(String target, RuleSet ruleSet) {
        RuleBean rule = (RuleBean) ruleSet.getRule(target);
        if (rule != null)
